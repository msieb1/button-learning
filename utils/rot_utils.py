...
def geodesic_dist_quat(q1, q2, tensor=True, sign=False):
    if tensor:
        dist = 2*torch.acos(torch.abs(torch.sum(q1*q2, dim=1)))
    else:
         if sign:
           dist = 2*np.arccos((np.sum(q1*q2)))
         else:
           dist = 2*np.arccos(np.abs(np.sum(q1*q2)))
    return dist
        

def norm_sincos(sin, cos, tensor=True):
    if tensor:
        stacked_ = torch.cat((sin[None], cos[None]))
        stacked = stacked_ / torch.norm(stacked_)
    else: 
        stacked_ = np.concatenate((sin[None], cos[None]))
        stacked = stacked_ / np.linalg.norm(stacked_)
